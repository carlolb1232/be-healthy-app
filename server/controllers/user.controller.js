...
module.exports.Register = async (req, res) => {
  try {
    ...
    return res.cookie("usertoken", jwtToken, process.env.SECRET_KEY, { httpOnly: true })
      .json({ message: "", email: user.email, _id: user._id, rol:user.rol })
  } catch (err) {
...
module.exports.getUser = async (req, res) => {
  try {
    const { id } = req.params
    const { email, firstName, lastName, _id, rol, periods, weight, height, imc, age } = await User.findById(id).exec();
    res.json({ email, firstName, lastName, _id, rol, periods, weight, height, imc, age })
  } catch (err) {
    return { success: false, data: err.message }
  }
}
